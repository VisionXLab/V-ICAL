...
class FlappyBirdEnv(gym.Env):
    ...
    def reward(self) -> SupportsFloat:
        if any([not pipe.passed and ((pipe.x+pipe.pipe_top.get_width()) < self._bird.x)
    ...
    def step(self, action: ActType) -> \
        ...
        for pipe in self._pipes:
            ...
            if not pipe.passed and (pipe.x+pipe.pipe_top.get_width()) < self._bird.x:
                self._score += 1
                pipe.passed = True
                add_pipe = True
        ...
        if add_pipe:
            self._pipes.append(Pipe(500, self.np_random))
    ...
    def reset(self, *, seed: int | None = None,
        ...
        self._pipes = [Pipe(400, self.np_random)]
        self._base = Base(700)
