...
class CaveFlyerGame : public BasicAbstractGame {
  public:
    ...
    void game_reset() override {
        ...
        initial_distance=sqrt(pow(agent->x-ent->x,2)+pow(agent->y-ent->y,2));
    ...
    void game_step() override {
        ...
        for (int ent_idx = (int)(entities.size()) - 1; ent_idx >= 0; ent_idx--) {
            ...
            if(ent->type==GOAL)
            {
                ...
                {
                    step_data.reward=(1-last_reward);
                }
                else
                {
                    float cur_score=1-sqrt(pow(agent->x-ent->x,2)+pow(agent->y-ent->y,2))/initial_distance;
                    float cur_reward=(cur_score-last_reward);
                    if (cur_reward>0)
                    {
                        step_data.reward=cur_reward;
                        last_reward=cur_score;
                    }
                    else
                    {
                        step_data.reward=0;
