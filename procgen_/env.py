...
class BaseProcgenEnv(CEnv):
    ...
    def __init__(
        ...
        resource_root = os.fsencode(resource_root)

        library_names = ["libenv.so", "libenv.dylib", "env.dll"]

        def has_environment_library(path):
            return any(
                os.path.isfile(os.path.join(path, name))
                for name in library_names
            )

        prebuilt_dir = os.path.join(SCRIPT_DIR, "data", "prebuilt")
        build_type = "debug" if debug else "relwithdebinfo"
        source_build_dir = os.path.join(SCRIPT_DIR, ".build", build_type)
        if platform.system() == "Windows":
            source_build_dir = os.path.join(source_build_dir, build_type)

        if has_environment_library(prebuilt_dir):
            lib_dir = prebuilt_dir
            assert not debug, "debug has no effect for pre-compiled library"
            print("Using pre-built .dll")
        elif has_environment_library(source_build_dir):
            # 复用已完成的源码构建。此前这里只检查 data/prebuilt，导致每个新
            # Python 进程首次创建 Procgen 时都会重新调用 CMake/MSBuild。
            lib_dir = source_build_dir
            print("Using existing source-built environment library")
        else:
