"""
Non-Video Ablation Batch Evaluation Tool
...
_NONVIDEO_PROMPT_REPLACEMENTS = (
    (
        "Watch the video and guess the rule, and play another round.",
        "Plan your next action based on the current frame.",
    ),
    (
        "Watch the video, guess the rule and play another round.",
        "Plan your next action based on the current frame.",
    ),
    (
        'Follow this "swing" strategy demonstrated in the video:',
        'Follow this "swing" strategy:',
    ),
)


def _adapt_prompt_for_nonvideo(prompt: str) -> str:
    """Remove demo-video-specific wording from a non-video prompt."""
    for source, replacement in _NONVIDEO_PROMPT_REPLACEMENTS:
...
async def batch_run_loop(ai: AIPlayer, session: GameSession) -> dict:
    ...
    def _record_usage(usage: dict, round_num: int):
        ...
        for k in ("input", "output", "thoughts", "total",
                   ...
                   "input_image_count", "input_video_count"):
    ...
    # nonvideo 消融：消除 prompt 中依赖 demo 视频的措辞
    context_prompt = _adapt_prompt_for_nonvideo(context_prompt)
    instruction_prompt = _adapt_prompt_for_nonvideo(instruction_prompt)
...
async def run_single_eval(
    ...
    *,
    ...
    # --- 3. nonvideo 消融：不加载 context（不传 demo 视频/图片） ---
    context = False
